...
document.getElementById("search-area").addEventListener("change", () => {
  console.log("CCC");
  let city = document.getElementById("search-area").value;
  if (city != 0) {
    console.log(city);

    let areaUrl =
      "https://apis.data.go.kr/B551011/KorService1/areaCode1?areaCode=" +
      city +
      "&serviceKey=" +
      serviceKey +
      "&numOfRows=20&pageNo=1&MobileOS=ETC&MobileApp=AppTest&_type=json";
...
document.getElementById("btn-search").addEventListener("click", () => {
  ...
  fetch(searchUrl)
    ...
    .then((data) => {
      ...
      let str = `serviceKey=${serviceKey}&numOfRows=20&pageNo=1&MobileOS=ETC&MobileApp=AppTest&_type=json&listYN=Y&arrange=A&mapX=${trips[0].mapx}&mapY=${trips[0].mapy}&radius=500`;
      if (parseInt(contentTypeId)) str += `&contentTypeId=${contentTypeId}`;
      let url = baseUrl + "locationBasedList1?" + str;
      console.log(url);
      fetch(url)
        .then((response) => response.json())
        .then((d) => makeList(d))
...
kakao.maps.event.addListener(map, "dragend", function () {
  ...
  let str = `serviceKey=${serviceKey}&numOfRows=20&pageNo=1&MobileOS=ETC&MobileApp=AppTest&_type=json&listYN=Y&arrange=A&mapX=${latlng.getLng()}&mapY=${latlng.getLat()}&radius=500`;
  if (parseInt(contentTypeId)) str += `&contentTypeId=${contentTypeId}`;
  let url = baseUrl + "locationBasedList1?" + str;
  fetch(url)
    .then((response) => response.json())
